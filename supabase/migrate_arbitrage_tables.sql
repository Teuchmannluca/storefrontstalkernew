-- Migration script to update arbitrage tables for scan functionality

-- First, backup the old arbitrage_opportunities table if it exists
DO $$
BEGIN
    IF EXISTS (
        ...
        -- Check if the old structure exists (has storefront_id column)
        IF EXISTS (
            SELECT FROM information_schema.columns 
            WHERE table_schema = 'public' 
            AND table_name = 'arbitrage_opportunities'
            AND column_name = 'storefront_id'
        ) THEN
            -- Rename the old table to backup
            ALTER TABLE arbitrage_opportunities RENAME TO arbitrage_opportunities_old_backup;
            RAISE NOTICE 'Old arbitrage_opportunities table backed up as arbitrage_opportunities_old_backup';
        END IF;
    END IF;
END $$;

-- Now create the new tables with scan functionality
...
-- Create table for scan opportunities/results (new structure)
...
UNION ALL
SELECT 'Old backup table exists: ' || 
  CASE WHEN EXISTS (
    SELECT FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name = 'arbitrage_opportunities_old_backup'
  ) THEN 'YES' ELSE 'NO' END;