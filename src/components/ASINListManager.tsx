...
interface ASINListManagerProps {
...
export default function ASINListManager({ 
  onLoadList, 
  onScanList,
  currentListId 
}: ASINListManagerProps) {
  ...
  const toggleFavorite = async (list: ASINList) => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return

      const response = await fetch(`/api/asin-lists/${list.id}`, {
        method: 'PUT',
        headers: {
  ...
  const deleteList = async (listId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/asin-lists/${listId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${session.access_token}`
        }
  ...
  const duplicateList = async (list: ASINList) => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">No ASIN Lists Yet</h3>
          <p className="text-gray-500 text-sm">Save your current ASINs as a list to reuse them later</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-900">My ASIN Lists</h2>
        <button
          onClick={fetchLists}
